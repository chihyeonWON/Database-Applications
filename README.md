...
트랜잭션이 읽고 쓰기를 할 때 사용하는 락 read-write 

데이터에 락이 걸려있지 않으면 트랜잭션은 데이터에 락을 걸 수 있음

회복 recovery

DB장애 시 DB를 일관성 있는 상태로 되돌리는 DBMS 기능

장애유형

시스템사고, 미디어장애, 응용소프트웨어오류, 자연재해, 부주의혹은 태엽

Rollback : undo 취소하고 되돌아가는거 이전상태로 돌아가는거
Rollforward : redo 재실행

롤백하면 그 이후에는 commit된것들만 남아있다.
commit안된것들은 redo를 통해서 재실행해야 한다.

회복관리자 : 원자성과 지속성 Atomixity, Duration

로그는 버퍼에 있다가 주기억장치에 로그파일이 기록된다.

즉시갱신

지연갱신

즉시갱신, 지역갱신에 따라 회복하는 수단이 달라진다.

dbms가 즉시 갱신하는 경우 commit전에 5,6이 db에 기록될 수 있음

지연갱신 : partially commited
완료전,후 실패는 aborted 원래 상태로 되돌아간다.

로그파일 잃어버린것을 복구시키기 위해서 사용
안전한 하드디스크에 저장됨
전원과 관계가 없음

로그의 구조

트랜직션번호, 로그의타입, 데이터항목이름, 수정전값, 수정후값

T1, {START, UPDATE, COMMIT, DELETE, ABORTED} , Customer(박지성).balance, 100000, 90000>
데이터 수정전, 후값

트랜직션번호, 타입, 항목이름, 수정전,후값

3,4번에 한거를 즉시즉시 로그파일에 저장하게된다. crash되었을 때 활용
...
즉시실행일때 checkpoint 이후에 commit을 한경우 redo를 한다.
commit을 안한 경우 undo를 한다.
트랜잭션을 모두 취소해야한다. 원상복구해야한다.
기록되어있을 가능성이 있음

갱신데이터 로그가 끝난 후 부분완료하고 버퍼->데이터베이스 작업이 진행되는 방법

t1을 취소 commit하기전이니까 undo

5에서 9는 t1은 commit했으니까 redo 재수행, t2는 commit이 없기 때문에 undo
...
commit했다고하더라도 완벽하게끝났지않았기때문에 재실행해야함 commit -redo commitx - undp

지연갱신 commit안했으면 noundo 아무것도 필요없음
commit을 안하면 디스크에 write될 가능성이 없기때문에 아무것도 할 필요가 없다.

t1~t6

t4 t5는 체크포인트 이후에 commit 즉시일때 redo 없으면 t1 t6는 undo
t4 t5는 체크포인트 이후에 commit 지연일때 redo 없으면 아무것도안해도됨됨

즉시 갱신 - commit이 있으면 redo commit이 없으면 undo
지연 갱신 - commit이 있으면 x commit이 없으면 undo

체크포인트를 이용한 회복

갱신하는 시간을 단축하기 위함
로그를 이용한 회복, 검사점 동기화한 시점을 로그파일에 기록해두는 방법 혹은 그 시점

체크포인트 이전에 commit 기록이 경우 - 아무작업x
체크포인트 이후에 commit 기록이 있는 경우 - redo
체크포인트 이후에 commit 기록이 없는 경우 - 즉시(undo), 지연갱신(할필요없음)
